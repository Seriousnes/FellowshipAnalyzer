...
public sealed class ThunderCallWindowAnalyzerTests
{
    ...
    public async Task Analyze_ThunderCallWindow_ExposesPerPullReadPaths()
    {
        ...
        var entry = parser.ThunderCallWindowAnalyzers.ShouldHaveSingleItem();
        var pull = entry.Pull;
        pull.Index.ShouldBe(0);

        pull.ThunderCallWindowAnalyzer.ShouldBeSameAs(entry.Analyzer);
        parser.For(pull).ThunderCallWindowAnalyzer.ShouldBeSameAs(entry.Analyzer);
    }
