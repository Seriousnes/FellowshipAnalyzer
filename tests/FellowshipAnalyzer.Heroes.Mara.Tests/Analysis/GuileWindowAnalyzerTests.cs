...
public sealed class GuileWindowAnalyzerTests
{
    ...
    public async Task Analyze_WithoutTheAssassinsGuileTalent_LeavesTheSurfaceEmpty()
    {
        ...
        parser.GuileWindowAnalyzers.ShouldBeEmpty();
        parser.StealthWindowAnalyzers.ShouldNotBeEmpty();
        parser.Pulls.ShouldHaveSingleItem().GuileWindowAnalyzer.ShouldBeNull();
    }
    ...
    public async Task Analyze_ExposesPerPullReadPaths()
    {
        ...
        var entry = parser.GuileWindowAnalyzers.ShouldHaveSingleItem();
        var pull = entry.Pull;

        pull.GuileWindowAnalyzer.ShouldBeSameAs(entry.Analyzer);
        parser.For(pull).GuileWindowAnalyzer.ShouldBeSameAs(entry.Analyzer);
    }
    ...
    private static async Task<GuileWindowAnalyzer> AnalyzeAsync(List<Event> events, ReportFight? fight = null)
    {
        var parser = await AnalyzeParserAsync(events, fight ?? BossFight());
        return parser.GuileWindowAnalyzers.ShouldHaveSingleItem().Analyzer;
    }
