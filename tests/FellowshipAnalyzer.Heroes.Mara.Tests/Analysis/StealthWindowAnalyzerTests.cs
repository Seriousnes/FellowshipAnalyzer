...
public sealed class StealthWindowAnalyzerTests
{
    ...
    public async Task Analyze_PoisonBeyondTheLinkHorizon_IsNotCreditedToTheWindow()
    {
        var events = new List<Event>
        {
            ...
            Buff<RemoveBuffEvent>(3000, Spells.BroodingShadowsBuff),
            Debuff<ApplyDebuffEvent>(1200 + StealthWindowAnalyzer.PoisonLinkWindowMs + 1, Spells.WidowBitePoison),
        };
    ...
    public async Task Analyze_ExposesPerPullReadPaths()
    {
        ...
        var entry = parser.StealthWindowAnalyzers.ShouldHaveSingleItem();
        var pull = entry.Pull;

        pull.StealthWindowAnalyzer.ShouldBeSameAs(entry.Analyzer);
        parser.For(pull).StealthWindowAnalyzer.ShouldBeSameAs(entry.Analyzer);
    }
    ...
    private static async Task<StealthWindowAnalyzer> AnalyzeAsync(List<Event> events, ReportFight? fight = null)
    {
        var parser = await AnalyzeParserAsync(events, fight ?? BossFight());
        return parser.StealthWindowAnalyzers.ShouldHaveSingleItem().Analyzer;
    }
