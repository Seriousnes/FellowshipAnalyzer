...
public sealed class FatedStrikeTrackerTests
{
    ...
    public async Task Analyze_NoWindowAndNoActivation_ContributesNoStatisticsCard()
    {
        ...
        var tracker = parser.FatedStrikeTracker.ShouldNotBeNull();
        tracker.Windows.ShouldBe(0);
        tracker.FatedStrikeCasts.ShouldBe(0);
        tracker.Statistic.ShouldBeNull();
        result.Statistics.ShouldNotContain(entry => entry.Module is FatedStrikeTracker);
    }
    ...
    public async Task Analyze_WithAWindow_ContributesTheStatisticsCard()
    {
        ...
        result.Statistics.ShouldContain(entry => entry.Module is FatedStrikeTracker);
    }
    ...
    private static async Task<FatedStrikeTracker> AnalyzeAsync(List<Event> events)
    {
        var (parser, _) = await RunAsync(events, BossFight(PullEnd));
        return parser.FatedStrikeTracker.ShouldNotBeNull();
    }
