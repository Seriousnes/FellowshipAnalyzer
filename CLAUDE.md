...
**Spell data** - `data/spelldb.json` is produced offline by `FellowshipAnalyzer.SpellData` and turned into per-hero `Spells` registries by the source generator. Curate `data/overrides.json` (via SpellStudio or by hand) and rerun `rebuild-spelldb` rather than editing generated registries. Per-hero `Talents` classes and the `{Hero}Talents` id constants are generated from the same file's `talents` section, which the merge builds by joining each `talent-slot` record to the talent it names, so only the slot supplies the hero. Spell ids are `FSLID` values, which offset effect, talent, and weapon ids into their own million-ranges.

The merge reads the highest-numbered `data/v*` export folder (`entities.jsonl` plus `settings.json`). A hero's kit is every ability record whose `heroes` array names that hero; an effect joins that ability through `partOf` and takes its member name from the ability's member name plus its `role`. An effect with a `partOf` but no `role` is not generated and belongs in `data/overrides.json`. The export is read through the `Fellowship.SDK` package, which supplies the document shape for every `$type`; nothing hand-parses `entities.jsonl`. Icons come from the export too, keyed by kind and native id, and name `.png` files. The codex CDN serves every texture under `/ui` with the size in the file name, so `Codex.IconUrl` writes that name and drops any directory the icon carries. A texture drawn once per rarity rung is addressed by a name ending in the rung's stored name (`-champion` for the rung printed as Heroic), which `data/spelldb.json`'s `rarities` map supplies. Item and gem textures the build shares across every rung keep their bare name; `spelldb.json`'s `texturesSharedAcrossRungs` list names them, and `ItemTexture.IsDrawnPerRung` reads that list, so a texture the export has never declared reads as drawn per rung. An entity the export gives no texture for takes its icon from `data/overrides.json`.
