...
3. Add the `HeroNameExtensions.ToHeroId` arm, the `Hero.IconUrl` arm (an `assets.fellows.gg` portrait URL, which the codex export carries no texture for), and the `Hero.Color` arm returning `FaVar.Hero{Name}`.
