...
public sealed class ConsolidatedSpellRegistryGenerator : IIncrementalGenerator
{
    ...
    private const string LegendariesSection = "legendaries";
    private const string TexturesSharedAcrossRungsSection = "texturesSharedAcrossRungs";
    private const string DungeonsSection = "dungeons";
        ...
        Compilation compilation)
    {
        ...
        List<JsonValue>? sharedTextureEntries = null;
        Dictionary<string, JsonValue>? talentHeroes = null;
        Dictionary<string, JsonValue>? legendaryHeroes = null;
        Dictionary<string, JsonValue>? dungeonEntries = null;

        foreach (var scopeName in SortedKeys(scopes))
        {
            if (string.Equals(scopeName, TexturesSharedAcrossRungsSection, StringComparison.Ordinal))
            {
                sharedTextureEntries = scopes[scopeName].Array;
                continue;
            }
            ...
            if (string.Equals(scopeName, LegendariesSection, StringComparison.Ordinal))
            {
                legendaryHeroes = scopeObj;
                continue;
            }

            if (string.Equals(scopeName, DungeonsSection, StringComparison.Ordinal))
            {
                dungeonEntries = scopeObj;
                continue;
            }
        ...
        if (sharedTextureEntries is not null)
            EmitItemTexture(spc, sharedTextureEntries);

        if (dungeonEntries is not null)
            EmitDungeons(spc, dungeonEntries);

        if (talentHeroes is not null)
            EmitTalents(spc, talentHeroes, kindTypes["talent"], schema, heroNames);

        if (legendaryHeroes is not null)
            EmitLegendaries(spc, legendaryHeroes, heroNames);
    }

    /// <summary>
    /// Emits the dungeon id → texture lookup, as a switch over the id. Fellowship Logs indexes its zone
    /// encounters by the same id the export gives a dungeon.
    /// </summary>
    private static void EmitDungeons(SourceProductionContext spc, Dictionary<string, JsonValue> entries)
    {
        ...
        sb.AppendLine("/// The texture each dungeon is drawn with, generated from <c>data/spelldb.json</c> and keyed by the");
        sb.AppendLine("/// id Fellowship Logs writes as a report's zone encounter.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public static class Dungeons");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// The texture for the dungeon at <paramref name=\"id\"/>, or <see langword=\"null\"/> when the");
        sb.AppendLine("    /// export declares no such dungeon.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    public static string? IconFor(int id) => id switch");
        sb.AppendLine("    {");

        foreach (var key in entries.Keys.OrderBy(k => int.TryParse(k, NumberStyles.Integer, CultureInfo.InvariantCulture, out var n) ? n : int.MaxValue))
        {
            if (!int.TryParse(key, NumberStyles.Integer, CultureInfo.InvariantCulture, out var id))
                continue;
            if (entries[key].String is not { } icon || string.IsNullOrWhiteSpace(icon))
                continue;

            sb.AppendLine("        " + id.ToString(CultureInfo.InvariantCulture)
        ...
        spc.AddSource(HintName(GameNamespace, "Dungeons"), sb.ToString());
    }

    /// <summary>
    /// Emits the test for a texture the build draws once for every rarity rung, as a switch over the
    /// texture name. Textures drawn per rung outnumber them many times over, so an unlisted name reads
    /// as drawn per rung.
    /// </summary>
    private static void EmitItemTexture(SourceProductionContext spc, List<JsonValue> entries)
    {
        var shared = entries
        ...
        var texture = new StringBuilder();
        texture.AppendLine("// <auto-generated />");
        texture.AppendLine("#nullable enable");
        texture.AppendLine();
        texture.AppendLine("namespace " + GameNamespace + ";");
        texture.AppendLine();
        texture.AppendLine("/// <summary>");
        texture.AppendLine("/// How the build draws item and gem textures, generated from <c>data/spelldb.json</c>.");
        texture.AppendLine("/// </summary>");
        texture.AppendLine("public static class ItemTexture");
        texture.AppendLine("{");
        texture.AppendLine("    /// <summary>");
        texture.AppendLine("    /// Whether <paramref name=\"texture\"/> is drawn once per rarity rung, and so is addressed by");
        texture.AppendLine("    /// a name ending in the rung's stored name. A texture shared across every rung is not.");
        texture.AppendLine("    /// </summary>");
        texture.AppendLine("    public static bool IsDrawnPerRung(global::System.ReadOnlySpan<char> texture) => texture switch");
        texture.AppendLine("    {");
        foreach (var name in shared)
            texture.AppendLine("        \"" + name.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\" => false,");
        texture.AppendLine("        _ => true,");
        texture.AppendLine("    };");
        texture.AppendLine("}");

        spc.AddSource(HintName(GameNamespace, "ItemTexture"), texture.ToString());
    }
    ...
    private static void EmitRarities(SourceProductionContext spc, Dictionary<string, JsonValue> entries)
    {
        ...
        sb.AppendLine("/// than the one it prints, because item and gem texture files end in it.");
        sb.AppendLine("/// </summary>");
    ...
    private const string ItemTypeGlobalName = "global::FellowshipAnalyzer.Core.Common.Items.Item";

    /// <summary>
    /// Turns the <c>legendaries</c> section (hero → member → entry) into one <c>Legendaries</c> class of
    /// static <c>Item</c> members per hero, named for the power each item grants, plus the companion
    /// <c>{Hero}Legendaries</c> item id constants.
    /// </summary>
    private static void EmitLegendaries(
        SourceProductionContext spc,
        Dictionary<string, JsonValue> heroes,
        HashSet<string>? heroNames)
    {
        foreach (var heroKey in SortedKeys(heroes))
        {
            ...
            var scopeLabel = LegendariesSection + "." + heroKey;
            var entries = new List<LegendaryEntry>();
            ...
            foreach (var memberName in SortedKeys(heroObj))
            {
                ...
                if (!entry.TryGetValue("itemId", out var itemIdValue) || itemIdValue.Number is not { } itemId)
                {
                    spc.ReportDiagnostic(Diagnostic.Create(MissingIdDescriptor, Location.None, memberName, scopeLabel));
                    continue;
                }

                var powerName = entry.TryGetValue("powerName", out var nameValue) ? nameValue.String ?? memberName : memberName;
                var icon = entry.TryGetValue("icon", out var iconValue) ? iconValue.String ?? "" : "";

                entries.Add(new LegendaryEntry(memberName, (int)Math.Round(itemId), powerName, icon));
            }

            if (entries.Count == 0)
                continue;

            EmitLegendaryClass(spc, hero, entries);
            EmitLegendaryConstants(spc, hero, entries);
        }
    }

    private static void EmitLegendaryClass(SourceProductionContext spc, string hero, List<LegendaryEntry> entries)
    {
        var ns = SpellsNamespace + "." + hero;
        ...
        sb.AppendLine("namespace " + ns + ";");
        sb.AppendLine();
        sb.AppendLine("/// <summary>" + hero + "'s legendary items, generated from <c>data/spelldb.json</c>. Each member is named for the power its item grants and holds the item id.</summary>");
        sb.AppendLine("public static partial class Legendaries");
        sb.AppendLine("{");
        foreach (var entry in entries)
        {
            sb.AppendLine("    /// <summary>" + hero + "'s <c>" + entry.PowerName + "</c> legendary.</summary>");
            sb.AppendLine("    public static " + ItemTypeGlobalName + " " + entry.Member + " { get; } = new(" +
                          entry.ItemId.ToString(CultureInfo.InvariantCulture) + ", " +
                          Literal(entry.PowerName) + ", " + Literal(entry.Icon) + ");");
            sb.AppendLine();
        }
        sb.AppendLine("}");

        spc.AddSource(HintName(ns, "Legendaries"), sb.ToString());
    }

    private static void EmitLegendaryConstants(SourceProductionContext spc, string hero, List<LegendaryEntry> entries)
    {
        var className = hero + "Legendaries";
        ...
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Compile-time item ids for <see cref=\"global::" + SpellsNamespace + "." + hero + ".Legendaries\"/>,");
        sb.AppendLine("/// generated so an attribute can name a legendary as a constant.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public static class " + className);
        sb.AppendLine("{");
        foreach (var entry in entries)
        {
            sb.AppendLine("    /// <summary>Item id of <c>" + entry.Member + "</c>.</summary>");
            sb.AppendLine("    public const int " + entry.Member + " = " +
                          entry.ItemId.ToString(CultureInfo.InvariantCulture) + ";");
        }
        sb.AppendLine("}");
    ...
    private readonly struct LegendaryEntry(string Member, int ItemId, string PowerName, string Icon)
    {
